...
def print_cnv(busco_df, busco_id, homology_groups):
    # print(busco_df.loc[busco_df["# Busco id"] == busco_id].squeeze())

    buscoid_df = busco_df.loc[busco_df["# Busco id"] == busco_id]

    # print(buscoid_df.iloc[0])
    print("Starting...")
    for item in buscoid_df.iloc[0, 1:]:
        ...
        if pd.isna(item):
            ...
            print(grep_first(homology_groups, item))
...
# def busco_groups_numbering(homology_groups, buscoid_df, busco_ID):
#
#
...
def main():
    ...
    print_cnv(busco_df, ID, homology_groups)
    # print(buscoid_df)
