...
Description: This script searches all potential GOIs based on BLAST in a
pangenome and makes a TSV file including the following information:
gen ID, homology group number, genome number, BLAST location, closest gene in AT
, the ID\% and name of this AT hit, Best hit in the Korso (T22) genome, ID\% of
the best Korso hit, YES/NO if the BLAST hit was found using the initial query or
 using the best BLASTP hit from the pangenome,
 and from which species the potential GOI is.

It starts by doing a BLASTP to the pangenome using an input query and makes a
list of all the genes which passed the BLAST. Then it that the best BLAST hit
and does a new BLASTP to the pangenome and also makes a list of all the genes
which passed the BLAST. Then it compares the two lists and takes the longest
list to get all the above-mentioned information for. To get the closest hit in
arabidopsis all the genes passing the BLAST are BLASTed to the ARAPORT proteome.
The name of this gene is substracted from the ARAPORT gff file. Other
information is extracted from PanTools files like homology grouping file etc.
For the name of the species and the genome number of the genes not blasted but
sharing a homology group with a blasted gene a separate file has to be made.
This is a TSV file containing a unique part o the gene names, genome number
in the pangenome, and the species name. Example of one line:
T25     _genome_2       cauliflower
- 'T25' is in every gene name of the T25 accession.
- T25 is the second genome in the pangenome. '_genome_' should be added before
the actual genome number to make it work.
- species name

Usage: python3 Variation_table.py {proteome/pangenome_database} {input query}
{desired sequence similarity to pass the BLAST}
{desired alignment length to pass the BLAST}
{gene labels} {arabidopsis gff file} {arabidopsis proteome}
{cauliflower (T22) proteome}
Example: nice python3 Variation_table.py pangenome_DB/ ELF3_AT2G25930_protein.faa 25 65 gene_labels_cauliflower.tsv AT_Korso/Araport11_GFF3_genes_transposons.current.filtered.gff AT_Korso/Araport11_GFF3_genes_transposons.current.filtered.pep.faa AT_Korso/proteins_15.fasta

Note: You need to have a pangenome with grouping function already run.
"""
...
def get_description(gene):
    ...
    #TODO: I would change the absolute path to the gff file so you do not need
    # to add it to the command all the time. After that comment out the argv
...
def output_writer(table_dic, output_file, best_ara_hits, best_cau_hits, first_blastids, second_blastids, blasted=False):
    ...
    # Following lines checks wheter a gene is found using the initial input
    # query or using the best BlAST hit
    found_query = []
    found_best_hit = []
    # gets only the gene ids from all the genes passing the initial BLAST
    # and the BLAST using the best pangenome hit
    ...
    # checks if the gene ids are in the list with the genes passing the BLAST(s)
    for key in table_dic.keys():
    ...
    # appends pd.df with YES/NO which genes are found with which of both BLAST
    ...
    # appends pd.df with the species names
    species = add_species(table_dic.keys(), argv[5])
    df["Species"] = species

    # makes a TSV file of the pd.df
    df.to_csv(output_file, sep='\t', index=False)


def change_notblasted(not_blasted, labels):
    """
    gets the genome numbers of all the genes not passing the blast but sharing
    a homology group with a gene which did pass the BLAST

    :param not_blasted: list, gene ids of not blasted genes
    :param labels: file, containing gene id, genome num, species name
    :return: list, gene id including '_genome_' and the genome number
    """
    ...
    for item in not_blasted:
        for line in lines:
            ...
            if line[0] in item:
                not_blasted_id_genomes.append(item + line[1])
                continue

    return not_blasted_id_genomes


def add_species(ids, labels):
    """
    gets the species name for every gene using the gene id

    :param ids: list, gene ids
    :param labels: file, containing gene id, genome num, species name
    :return: list, with the species names in order of how they should occur
...
def main():
    ...
    if len(first_blast_ids) > len(second_blast_ids):# or argv[6] == 'yes':
    ...
    #TODO: you can add an argv here to name the ouput file, but this does take
    # an extra argument in the command
    output_path = pangenome_location.parent / "variation_table.tsv"
    ...
    not_blasted = change_notblasted(not_blasted, argv[5])
    if not not_blasted:
    ...
    # TODO: you can add an argv here to name the ouput file, but this does take
    #  an extra argument in the command
    output_notblasted_path = pangenome_location.parent / "variation_table_not_blasted.tsv"
